...
#include <sstream> // Para manejar strings
...
public:
    ...
    void crearArchivoDot(const std::string& filename) {
        std::ofstream file(filename);
        if (!file.is_open()) {
            std::cerr << "No se pudo abrir el archivo " << filename << " para escritura.\n";
            return;
        }
    ...
    void generarDot2(Node<T>* node, std::ofstream& file) {
        if (node != nullptr) {
            file << "node_" << node << " [label=\"";
            for (int i = 0; i < node->size; ++i) {
                file << "<f" << i << "> " << node->item[i];
                if (i < node->size - 1) {
                    file << " | ";
                }
            }
            ...
            if (!node->is_leaf) {
                for (int i = 0; i <= node->size; ++i) {
                    if (node->children[i] != nullptr) {
                        file << "\"node_" << node << "\":f" << i << " -> \"node_" << node->children[i] << "\";\n";
                        generarDot(node->children[i], file);
                    }
                }
            }
        }
    ...
    void crearArchivoDot2(const std::string& filename) {
        std::ofstream file(filename);
        if (!file.is_open()) {
            std::cerr << "No se pudo abrir el archivo " << filename << " para escritura.\n";
            return;
    ...
    void generateGraphvizDot(const std::string& filename) {
        std::ofstream file(filename);
...
private:
    void generarDot(Node<T>* node, std::ofstream& file) {
        if (node != nullptr) {
    ...
    void generateGraphvizDot(Node<T>* cursor, std::ofstream& file) {
        if (cursor != nullptr) {
            // Escribir nodos y sus valores
            file << "node" << reinterpret_cast<uintptr_t>(cursor) << " [label=\"";
            for (int i = 0; i < cursor->size; ++i) {
                file << "<f" << i << "> |" << cursor->item[i] << "| ";
            }
            file << "\", shape=record];\n";
            ...
            if (!cursor->is_leaf) {
                for (int i = 0; i <= cursor->size; ++i) {
                    file << "node" << reinterpret_cast<uintptr_t>(cursor) << ":f" << i
                         << " -> node" << reinterpret_cast<uintptr_t>(cursor->children[i])
                         << ";\n";
                    generateGraphvizDot(cursor->children[i], file);
                }
            }
        }
    }
